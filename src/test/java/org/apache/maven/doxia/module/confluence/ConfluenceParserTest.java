    ...
    extends AbstractParserTest
{
        ...
        throws Exception
    {
        ...
        assertContainsLines( result, "begin:link, name: http://jira.codehaus.org\ntext: JIRA\n" );
        assertContainsLines( result, "begin:monospaced\ntext: {{{{{{{{greedy monospaced}}}}}}}\n" );
        
        // four paragraphs in the input...
        assertEquals( 6, result.split( "end:paragraph" ).length );
    }
        ...
        throws Exception
    {
        ...
        String check = "text:  testcase3 ";
        int start = result.indexOf(check)+check.length();
        assertEquals( "\nbegin:bold\ntext: bold \nbegin:monospaced\ntext: monospaced ${with.escaped.variable}\nend:monospaced\n\n\n",
        		result.substring( start, result.indexOf( "end:bold", start ) ) );

        check = "text:  testcase4 ";
        start = result.indexOf(check)+check.length();
        assertEquals( "and \nbegin:monospaced\ntext: pseudo-escaped\\\nend:monospaced\n\n\ntext:  path\n",
        		result.substring( start, result.indexOf( "end:paragraph", start ) ) );
        
        assertContainsLines( result, "begin:paragraph\ntext: A programmatic_identifier_not_italicized\nend:paragraph" );
        assertContainsLines( result, "begin:bold\ntext: bold programmatic_identifier_with_underscores\nend:bold" );
        assertContainsLines( result, "begin:bold\ntext: bold programmatic_identifier_with_underscores\nend:bold" );
        assertContainsLines( result, "begin:italic\ntext: italic_programmatic_identifier\nend:italic" );
        assertContainsLines( result, "begin:italic\ntext: _with__some__double__underscores_\nend:italic\n\n\ntext:  __and__escaped__\nend:paragraph" );
        
        
        // 8 paragraphs in the input...
        assertEquals( 8, result.split( "end:paragraph\n" ).length );
        // 8 bolds in the input...
        assertEquals( 8, result.split( "end:bold\n" ).length );
        // 7 italics in the input...
        assertEquals( 7, result.split( "end:italic\n" ).length );
        // 4 monospaced in the input...
        assertEquals( 4, result.split( "end:monospaced\n" ).length );
    }
    ...
    private void assertContainsLines( String message, String result, String lines )
    {
        ...
        {
            assertTrue( "lines not found ===\n" + lines + "\n====", result.indexOf( lines ) != -1 );
        }
    }
